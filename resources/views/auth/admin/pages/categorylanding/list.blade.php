        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="d-flex justify-content-between ">
                    <a href="{{route('admin.categorylanding.create',['parent_id'=>request()->parent_id??0])}}" class="btn btn-info btn-md mb-2">+ Thêm mới</a>
                ...
                <ol class="breadcrumb">
                  <li><a href="{{ route('admin.categorylanding.index',['parent_id'=>0]) }}">Root</a></li>
                  @foreach ($parentBr->breadcrumb as $item)
                   <li><a href="{{ route('admin.categorylanding.index',['parent_id'=>$item['id']]) }}">{{ $item['name'] }}</a></li>
                  @endforeach
                  <li><a href="{{ route('admin.categorylanding.index',['parent_id'=>$parentBr->id]) }}">{{ $parentBr->name }}</a></li>
                </ol>
                @endif

                <div class="card card-outline card-primary">
                    <div class="card-body table-responsive lb-list-category">
                        @include('admin.components.category-landing', [
                            'data' => $data,
                            'routeNameEdit'=>'admin.categorylanding.edit',
                            'routeNameAdd'=>'admin.categorylanding.create',
                            'routeNameDelete'=>'admin.categorylanding.destroy',
                            'routeNameOrder'=>'admin.loadOrderVeryModel',
                            'table'=>'category_landings',
                        ])
                    </div>
                </div>
            </div>
